...
void main() {
  group('BKDTree', () {
    test('insert single point with docID, tree is not empty', () {
      final tree = BKDTree();
      const point = GeoPoint(lat: 40.7128, lon: -74.0060);
      tree.insert(point, [1]);
    ...
    test('insert multiple points, all findable', () {
      ...
      const la = GeoPoint(lat: 34.0522, lon: -118.2437);
    ...
    test(
      'searchByRadius with inclusive=true includes boundary points',
      () {
        final tree = BKDTree();
        const origin = GeoPoint(lat: 0, lon: 0);
        const target = GeoPoint(lat: 1, lon: 0);
        ...
        expect(
          inclusive.any((r) => r.point == target),
          isTrue,
        );

        // inclusive=false: boundary point is NOT included
        // (only outside)
        final exclusive = tree.searchByRadius(
          origin,
          exactDist,
          inclusive: false,
        );
        expect(
          exclusive.any((r) => r.point == target),
          isFalse,
        );
      },
    );
    ...
    test(
      'haversineDistance between known coordinates matches expected',
      () {
        const nyc = GeoPoint(lat: 40.7128, lon: -74.0060);
        const la = GeoPoint(lat: 34.0522, lon: -118.2437);
    ...
    test(
      'vincentyDistance between known coordinates matches expected',
      () {
        const nyc = GeoPoint(lat: 40.7128, lon: -74.0060);
        const la = GeoPoint(lat: 34.0522, lon: -118.2437);
        ...
        // Vincenty should also be ~3,944 km for NYC-LA, slightly
        // different from Haversine due to ellipsoidal model.
        // Allow 1% tolerance.
        expect(distance, closeTo(3944000, 40000));
    ...
    test('searchByPolygon finds points inside polygon', () {
      ...
      // Points
      const nyc = GeoPoint(lat: 40.7128, lon: -74.0060);
      const philly = GeoPoint(lat: 39.9526, lon: -75.1652);

      tree
        ..insert(nyc, [1])
        ..insert(philly, [2]);

      // Polygon around the US Northeast that includes NYC
      // and Philly
      final polygon = [
        const GeoPoint(lat: 42, lon: -80),
        const GeoPoint(lat: 42, lon: -70),
        const GeoPoint(lat: 38, lon: -70),
        const GeoPoint(lat: 38, lon: -80),
      ];
    ...
    test('removeDocByID removes specific docID from point', () {
      ...
      tree
        ..insert(point, [1, 2, 3])
        // Remove one docID
        ..removeDocByID(point, 2);
      expect(
        tree.getDocIDsByCoordinates(point),
        unorderedEquals([1, 3]),
      );

      // Remove remaining docIDs — node should be removed
      tree
        ..removeDocByID(point, 1)
        ..removeDocByID(point, 3);
      expect(tree.contains(point), isFalse);
      expect(tree.getDocIDsByCoordinates(point), isNull);
    });

    test(
      'convertDistanceToMeters converts km, mi, etc correctly',
      () {
        // cm -> meters
        expect(
          BKDTree.convertDistanceToMeters(100, DistanceUnit.cm),
          1.0,
        );

        // m -> meters (identity)
        expect(
          BKDTree.convertDistanceToMeters(1, DistanceUnit.m),
          1.0,
        );

        // km -> meters
        expect(
          BKDTree.convertDistanceToMeters(1, DistanceUnit.km),
          1000.0,
        );
    ...
    test(
      'toJson/fromJson round-trip preserves tree structure',
      () {
        final tree = BKDTree();
        const nyc = GeoPoint(lat: 40.7128, lon: -74.0060);
        const la = GeoPoint(lat: 34.0522, lon: -118.2437);
